...
class DocumentTest < ActiveSupport::TestCase
  ...
  should "only allow one primary file per thesis, and it should ignore deleted" do
     ...
     assert ! build(:document, thesis: thesis, supplemental: false).valid?, "Already have one primary, so it should fails"          
     assert  build(:document, thesis: thesis, supplemental: false, deleted: true).valid?, "If its a deleted file it should be ok"
  end  
  ...
  should "upload non-image files without complaining" do
   ...
   assert_equal "/uploads/theses/#{d.thesis.id}/files/#{d.id}/html-document.html", d.file.url, "file should be uploaded"
