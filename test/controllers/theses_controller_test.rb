  ...
  context "as admin" do
    ...
    should "Show Thesis details or fail if thesis was not found" do
      ...
      assert_raises ActiveRecord::RecordNotFound, "Should throw a RecordNotFound" do
        get :show, params: { id: 29292929, student_id: @student.id }
      end
    ...
    should "prepopulate the thesis if gem record id is passed and student ids match" do
      ...
      assert_equal record.examdate.beginning_of_day, thesis.exam_date.beginning_of_day, "Exam date is prepopulate with event date"
    ...
    should "make sure that gem_record_event_id is saved" do
      ...
      post :create, params: { student_id: @student.id, thesis: thesis.attributes.except("id", "created_at", "updated_at", "status", "published_date") }
    ...
    should "update loc_subjects as part of the thesis update" do
      ...
      thesis = create(:thesis, student: @student)
      put :update, params: { id: thesis.id, student_id: @student.id, thesis: { title: "new title", author: "new author", status: Thesis::OPEN, loc_subject_ids: [loc_subject.id, loc_subject2.id]} }
      t = assigns(:thesis)
    ...
    should "update committee members as part of the thesis update" do
      ...
      put :update, params: { id: thesis.id, student_id: @student.id, thesis: { title: "new title",
          committee_members_attributes: {"0" => { name: "Test", role: "Chair"} } } }
      t = assigns(:thesis)
      assert t, "New thesis must not be nil"
    ...
    should "update a valid Thesis, Student can't be changed. Status can't be changed" do
      ...
      put :update, params: { id: thesis.id, student_id: @student.id, thesis: { title: "new title", author: "new author", status: Thesis::OPEN} }
    ...
    should "send an email if notification[student] or notifcation[current_user] are present" do
      ...
      post :update_status, params: { id: thesis.id, student_id: @student.id, status: Thesis::UNDER_REVIEW, notify_current_user: true, notify_student: true }
    ...
    should "not send email if notify_student is not set" do
      ...
      AppSettings.email_status_change_allow = true
      #ActionMailer::Base.deliveries = []
      post :update_status, params: { id: thesis.id, student_id: @student.id, status: Thesis::UNDER_REVIEW, notify_current_user: true, custom_message: "woot" }
  ...
  context "as a student" do
     setup do
      ...
      should "not be able to update immutable thesis fields" do
        ...
        post :update, params: { id: t.id, student_id: @student.id, thesis: { title: "new title", author: "new author" } }
      ...
      should "return to student view upload process if thesis is updated" do
        ...
        assert_redirected_to student_view_thesis_process_path(@thesis, Thesis::PROCESS_UPLOAD), "Should redirect to student view Upload process"
      end
      ...
      should "submit for review, thesis status will cahnge to under_review" do
        ...
        assert_redirected_to student_view_thesis_process_path(@thesis, Thesis::PROCESS_STATUS), "Should redirect to student thesis process Status page"
        ...
        assert_equal thesis.student_accepted_terms_at.beginning_of_day, Date.today.beginning_of_day, "Esnure terms accepted at is today"

        assert thesis.under_review_at != nil, "Ensure Under Review Dat was assigned"
        assert_equal thesis.under_review_at.beginning_of_day, Date.today.beginning_of_day, "Esnure under review at is today"
