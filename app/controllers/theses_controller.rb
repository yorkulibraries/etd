class ThesesController < ApplicationController
  ...
  def update_status
    ...
    if params[:status] && Thesis::STATUSES.include?(params[:status])
      ...
      if params[:notify_student].blank? == false
        additional_recipients =  params[:notify_current_user] ? [current_user.email] : Array.new
        custom_message ||= params[:custom_message]

        StudentMailer.status_change_email(@student, @thesis, old_status, @thesis.status, additional_recipients, custom_message).deliver_later
  ...
  def thesis_params
    params.require(:thesis).permit( :title, :gem_record_event_id, :author, :supervisor, :student,
                    ...
                    committee_members_attributes: [:first_name, :last_name, :role, :id, :_destroy],
                    loc_subject_ids: [])
  end
