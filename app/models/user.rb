class User < ApplicationRecord
  ...
  validates_format_of :username, :with => /^[-\w\._]+$/i, multiline: true, :allow_blank => false, :message => "should only contain letters, numbers, or .-_"
  validates_format_of :email, :with => /^[-a-z0-9_+\.]+\@([-a-z0-9]+\.)+[a-z0-9]{2,4}$/i, multiline: true, :message => "Email must follow address@domain format", :unless => :is_student?
  ...
  def self.find_active_user(criterion)
    ...
    if criterion =~ /^[0-9]+$/
      return Student.active.find_by_sisid(criterion)
    else
      return User.active.not_students.find_by_username(criterion)
    end
